...
public class BDao {
	...
	public int getTotalCount(Criteria cri) {
		...
		try {
			...
			if(cri.getSearchWord() == null)
				query = "select count(*) totalCount from mvc_board";
			else {
				if(cri.getSearchItem().equals("searchName"))
					query = "select count(*) totalCount from mvc_board where bName like \'%" + cri.getSearchWord() + "%\'";
				else if(cri.getSearchItem().equals("searchTitle"))
					query = "select count(*) totalCount from mvc_board where bTitle like \'%" + cri.getSearchWord() + "%\'";
				else if(cri.getSearchItem().equals("searchContent"))
					query = "select count(*) totalCount from mvc_board where bContent like \'%" + cri.getSearchWord() + "%\'";
			}
	...
	//page -------------------------------------
	//public ArrayList<BDto> getListWithPaging(String pageNum, String amount){
	public ArrayList<BDto> getListWithPaging(Criteria cri){
		...
		try {
			...
			if(cri.getSearchWord() == null) {
				String query = "select * from (select rownum as rnum, a.* from (select * from mvc_board order by bgroup desc, bstep asc) a where rownum <= ? * ?) where rnum > (?-1) * ?";
				// 예전 방법 , 커넥션 풀 안쓸때
				//Calss.forName(driver);
				...
				preSta.setString(4, String.valueOf(cri.getAmount()));
			}else {
				String query = null;
				...
				if(cri.getSearchItem().equals("searchName")) {
					...
					// NG
					//query = "select * from (select rownum as rnum, a.* from (select * from mvc_board where bTitle like \'%?%\' order by bgroup desc, bstep asc) a where rownum <= ? * ?) where rnum > (?-1) * ?";
					
					//OK
					query = "select * from (select rownum as rnum, a.* from (select * from mvc_board where bTitle like \'%" + cri.getSearchWord() + "%\' order by bgroup desc, bstep asc) a where rownum <= ? * ?) where rnum > (?-1) * ?";
					
					System.out.println("---27--- query:" + query);
					System.out.println("---28,--- [" + cri.getSearchWord() + "]");
					con = dataSource.getConnection();
					preSta = con.prepareStatement(query);
					...
					// NG
					//query = "select * from (select rownum as rnum, a.* from (select * from mvc_board where bContent like '%?%' order by bgroup desc, bstep asc) a where rownum <= ? * ?) where rnum > (?-1) * ?";
					// OK
					query = "select * from (select rownum as rnum, a.* from (select * from mvc_board where bContent like \'%" + cri.getSearchWord() + "%\' order by bgroup desc, bstep asc) a where rownum <= ? * ?) where rnum > (?-1) * ?";
					
					System.out.println("---28--- query:" + query);
...
//String query = "select * from mvc_board order by bgroup desc, bstep asc";
	...
	public void modify(String bid2, String bname2, String btitle2, String bcontent2) {
		...
		try {
			con = dataSource.getConnection();
			String query2 = "update mvc_board set bname=?,btitle=?,bcontent=? where bid=?";
	...
	public void modify2(String bid2, String bname2, String btitle2, String bcontent2) {
		// TODO Auto-generated method stub
